...
#create a dataset with woman as partner
tempA <- fixed %>% 
  mutate(genderE = gender.x, partnum = 1, 
         gender = ifelse(gender.x == 1, "A", "P")) %>%
  gather(variable, value, beds:vacuum) %>%
  unite(var_gender, variable, gender) %>%
  spread(var_gender, value)

#create another dataset with men as partner
#a for her and p for him
tempB <- acitelli %>% 
...
#bind them together
#make sure you did them correctly by having a checkerboard dataset

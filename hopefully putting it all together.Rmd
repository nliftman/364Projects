...
pairwise <- pairwise %>% 
  select(dyadID, partID, obsid, gender, day, day_of_study, beds:calls, vacuum, qmi4, ind_chores, cup_chores, daily_per)

#pivot longer baby
pairwise2 <- pairwise %>% 
  gather(variable, value, beds:daily_per)

#creating a variable that has both the variable and the gender
pairwise3 <- pairwise2 %>%
  unite(var_gender, variable, gender)

#spreading them back out
pairwise4 <- pairwise3 %>%
  select(-partID) %>%

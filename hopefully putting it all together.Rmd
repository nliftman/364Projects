...
#creating one last big boy
#this includes everything yay yay yay yay
...
```{r}
#making it so each has a easy gender thing to add to the end
pairwise <- total_chores %>% 
  mutate(gender = ifelse(gender == 'Woman', 'W', 'M'))

#rearranging the data
pairwise <- pairwise %>% 
  select(dyadID, partID, gender, day, day_of_study, beds:calls, vacuum, qmi4, ind_chores, cup_chores, daily_per)

#pivoting longer
pairwise2 <- pairwise %>%
  gather(variable, value, beds:daily_per)

#creating a new variable that adds the variable and gender
pairwise3 <- pairwise2 %>% 
  unite(var_gender, variable, gender)

#spreading them out, cant quite figure this out 
pairwise4 <- pairwise3 %>%

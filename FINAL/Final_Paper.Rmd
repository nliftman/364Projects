...
```
# Premeasures
## Cleaning and Creating Scores for Hostile and Benevolent Sexism
```{r ASI, include = FALSE}
# Loading the data 
...
# Benevolent sexism
benevolent <- pre %>% 
  select(dyadID, gender, asi_1, asi_3.r, asi_6.r, asi_8, asi_9, asi_12, asi_13.r, asi_17, asi_19, asi_20, asi_22)
# Benevolent correlation matrix
corr.test(select(benevolent, asi_1:asi_22))$r

# Hostile sexism
hostile <- pre %>% 
  select(dyadID, gender, asi_2, asi_4, asi_5, asi_7.r, asi_10, asi_11, asi_14, asi_15, asi_16, asi_21.r)
...
pre_fixed <- benevolent %>% 
  inner_join(hostile, by = c("dyadID", "gender"))
...
#Now that we have our own variables we need to make it into pairwise
pre_fixed2 <- pre_fixed %>% 
  mutate(gender = ifelse(gender == "Woman", "W", "M"))

#now we want to gather
pre_fixed3 <- pre_fixed2 %>% 
  select(dyadID, gender, bs, hs) %>% 
  gather(variable, value, bs:hs)
...
#spread it baby!!! HAVING ISSUES :(
pre5 <- pre4 %>%
  pivot_wider(names_from = var_gender, values_from = value)
```
## Creating a Pairwise Dataset for the Premeasures Data
...
analysis_data <- pre_fixed %>% 
  ...
  ungroup() %>%
  select(dyadID, partnum, asi_1:hs)

# Person 1 is actor
temp_A <- analysis_data %>%
  mutate(partnum2 = 1, partnum = ifelse(partnum == 1, "A", "P")) %>%
  gather(variable, value, asi_1:hs) %>%
  unite(var_partnum, variable, partnum) %>%
  select(-partnum2) %>%
  pivot_wider(names_from = var_partnum, values_from = value)
...
## Creating the Scores For Chores Performed and Total
...
## Making the Daily Pairwise
```{r pairwise-daily, include = FALSE}
# Creating a obsid
total_chores <- total_chores %>%
  mutate(obsid = day_of_study+14*(dyadID-1))

filtered_chores <- total_chores %>% 
  group_by(obsid) %>%
  count()

total_chores <- total_chores %>%
  full_join(filtered_chores, by = "obsid")
...
# Cleaning it up
daily_pairwise <- pairwise4 %>%
  select(-qmi4_NA, -daily_per_NA)
```

# Finding the Residual Error 
```{r}
#special additional argument that is added for distinguishable dyads
apim_di_int <- gls(qmi4 ~ 1,
                   data = acitelli_pair,
                   correlation = corCompSymm(form=~1|cuplid), 
                   weights = varIdent(form=~1|genderE_A), 
                   na.action = na.omit)

#to find mans residual error, you look to the bottom, but to find woman, you 
#take the estimate at -1 and multiply it by the mans
summary(apim_di_int)

apim_di_int <- gls(hs ~ 1,
                   data = acitelli_pair,
                   correlation = corCompSymm(form=~1|cuplid), 
                   weights = varIdent(form=~1|genderE_A), 
                   na.action = na.omit)

apim_di_int <- gls(bs ~ 1,
                   data = acitelli_pair,
                   correlation = corCompSymm(form=~1|cuplid), 
                   weights = varIdent(form=~1|genderE_A), 
                   na.action = na.omit)

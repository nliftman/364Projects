...
tab <- data.frame(
  ...
  'SD ' = c(1.01, 0.72),
  'Alpha' = c(.8, .87),
  'Gender Difference' = c('idk', 'i dont rememebr'),
  'Hostile Sexism' = c(1, '0.024'),
  'Benevolent Sexism' = c('0.36', 1))

new <- dyad_pair %>% 
  mutate(gender2_A = ifelse(gender2_A == 'Woman', -1, 1),
         gender2_A = as.numeric(gender2_A)) %>% 
  na.omit(hs_A)

t.test(hs_A ~ gender2_A, data = new, paired = TRUE)
...
tab %>%
  ...
  kable_minimal() %>% 
  add_header_above(c(" " = 1, "Men" = 2, "Women" = 2," "= 2, "Correnlations" = 2)) %>% 
...
corrtab <- data.frame(
  'Scales' = c('1X Male Hostile Sexism', '2X Male Benevolent Sexism', '3X Female Hostile Sexism', '4X Female Benevolent Sexism'),
...
corrtab %>%
  kbl(caption = "Table 1. Correlation Matrix Between Sexism Scores") %>% 
  kable_classic() %>% 
  add_header_above(c(" " = 3,  "Correlations" = 4)) %>% 
  footnote(general = "Possible scores for hostile and benevolent sexism ranged from 1 to 7. All correlations except for male hostile sexism with male benevolent sexism was statistically signficinat with p < .01")

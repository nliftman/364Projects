...
fixed <- merge(nomoreProblemshopefully, final, by = 'obsid') %>% 
  select(dyadID.x, obsid, gender.x, day, day_of_study, qmi4, daily_per)

# Adding gender labels and rearranging the data
pairwise <- fixed %>% 
  mutate(gender2 = gender.x, gender.x = ifelse(gender.x == 'Woman', 'A', 'P')) %>%
  select(dyadID.x, obsid, gender.x, day, day_of_study, qmi4, daily_per, gender2) %>% 
...
```{r}
#just trying to join them together
#cant quite get this to work and not quite sure why
dyad_pair <- bind_rows(pairwise4, pairwise8) %>%
  arrange(dyadID)

#wrong :(
dyad_pair <- merge(pairwise4, pairwise8, by = 'dyadID')

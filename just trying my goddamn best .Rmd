...
pairwise <- fixed %>% 
  ...
  select(dyadID.x, obsid, gender.x, day, day_of_study, qmi4, daily_per, bs, hs, gender2) %>% 
  rename(dyadID = dyadID.x)

# Pivoting longer
pairwise2 <- pairwise %>% 
  gather(variable, value, qmi4:gender2)
...
pairwise4$daily_per_A[is.na(pairwise4$daily_per_A)] = 0
pairwise4$daily_per_P[is.na(pairwise4$daily_per_P)] = 0
pairwise4$gender2_A[is.na(pairwise4$gender2_A)] = "Woman"
pairwise4$gender2_P[is.na(pairwise4$gender2_P)] = "Man"
...
#filling in the categories where we know what they should be 
pairwise8$daily_per_A[is.na(pairwise8$daily_per_A)] = 0
pairwise8$daily_per_P[is.na(pairwise8$daily_per_P)] = 0
pairwise8$gender2_A[is.na(pairwise8$gender2_A)] = "Man"
pairwise8$gender2_P[is.na(pairwise8$gender2_P)] = "Woman"
```

# Finally Making Everything Pairwise
```{r}
#joining the datasets together
dyad_pair <- bind_rows(pairwise4, pairwise8) %>%
  arrange(obsid)
...
# Now That Everything Is Pairwise We Gotta Bring it All Back Together Again
```{r}
#attempting to bring it all together pls pls pls pls pls pls pls work
newbaby <- dyad_pair %>% 
  right_join(forlater, by = c('dyadID', 'obsid', 'day'))
  
#looking at whats the deal with the repeating
letsdosomecounting <- newbaby %>% 
  group_by(obsid) %>% 
  count()

#looking at this specifc to try to learn more
newbaby <- newbaby %>% 
  filter(obsid == 13136524)
...
#groupby dyad and gender and then just pull out the first day 
#first row function, not eveyrone has day one 
